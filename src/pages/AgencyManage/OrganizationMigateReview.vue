  ...
  <div>
    ...
    <el-form :model="searchData" label-width="120px">
      <el-row>
        ...
        <el-col :span="24">
          <el-form-item label="">
            <el-button type="primary" @click="handleSearch" v-has="'ORGANI_MANAGE_AUDIT_REPEAT'">查询</el-button>
            <el-button type="success" :disabled="isDisabled || listId.mv_status === 4 || listId.mv_status === 5" @click="handleAuit(1)">复核通过</el-button>
            <el-button type="success" :disabled="isDisabled || listId.mv_status === 4 || listId.mv_status === 5" @click="handleAuit(2)">复核不通过</el-button>
            <el-button type="warning" :disabled="isDisabled" @click="handleAuit(3)">撤回复核</el-button>
            <el-button type="info" @click="handleReset" v-has="'ORGANI_MANAGE_AUDIT_REPEAT'">重置</el-button>
          </el-form-item>
        </el-col>
      </el-row>
    </el-form>
    <div class="common-padding">
      <el-table
        ...
        <el-table-column label="申请时间" prop="mv_time"></el-table-column>
        ...
        <el-table-column label="复核时间" prop="mv_audit_time2"></el-table-column>
...
export default {
  ...
  methods: {
    // 请求
    async getData () {
      let res = await this.$axios.send('ORGANI_MANAGE_AUDIT_REPEAT', this.searchData)
    ...
    handleAuit (num) {
      ...
      let name
      if (num === 1) { name = '复核通过' } else if (num === 2) { name = '复核不通过' } else { name = '撤回审核' }
      this.$confirm(`确定该组织迁移${name}`, '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(async () => {
        let res = await this.$axios.send('ORGANT_MANAGE_MOVEAUDITPASS_REPEAT', auitObj, {$mv_id: this.listId.mv_id})
        if (res.status_code === 0) {
          this.$message({
            type: 'success',
            message: '操作成功'
