  ...
  <div>
      ...
      <el-row>
        ...
        <el-col :span="8">
          <el-form-item label="主附险类型">
            <el-select v-model="searchData.search_property" clearable  placeholder="">
        ...
        <el-col :span="24">
          <el-form-item label="">
            ...
            <el-button type="warning"  @click="coefficientAdd" :disabled="!multipleSelection.length > 0" v-has="'PRODUCT_COEFFICIENT_UPDATE'">折标系数设置</el-button>
          </el-form-item>
        </el-col>
      </el-row>
    </el-form>
    ...
    <el-row>
      <el-col :span="24">
        ...
        <el-table :data="tableData.data" highlight-current-row border @current-change="tableItemSelect" @selection-change="handleSelectionChange">
          <el-table-column width="80" type="selection" fixed prop="name"></el-table-column>
          ...
          <el-table-column min-width="75"  label="折标系数方案">
            <template slot-scope="scope" v-if="scope.row.coefficient_set !== 0">
              <el-button type="text" @click="getDetail(scope.row.p_id)">查看</el-button>
            </template>
          </el-table-column>
        </el-table>
        </div>
    ...
    <base-dialog title="产品折标系数"  :isshow.sync="dialog" width="500px" height="auto">
      <section class="dialog-center log">
        <el-table
            :data="detailList"
            border
            style="width: 100%">
            <el-table-column  label="缴费年限">
                <template slot-scope="scope">
                  {{scope.row.year}} 年缴
                </template>
            </el-table-column>
...
export default {
  ...
  methods: {
    ...
    rateEdit (id) {
      id = Number.isInteger(id) ? id : this.multipleSelection[0].p_id
      this.$router.push({name: '产品佣金费率设置', params: {id: id}})
    },

    async getDetail (pid) {
      let ret = await this.$axios.send('PRODUCT_COEFFICIENT_DETAIL', {p_id: pid, rule_id: 1})
      if (ret && ret.status_code === 0) {
        this.detailList = ret.result.c_data
        this.dialog = true
      }
    },
    ...
    coefficientAdd () {
      ...
      if (this.multipleSelection.length === 1 && this.multipleSelection[0].coefficient_set === 1) {
        this.coefficientUpdate(this.multipleSelection[0])
        return
      }
    ...
    async showDetail (id) {
      let ret = await this.$axios.send('PRODUCT_RATE_DETAIL', '', {p_id: id})
      if (ret && ret.status_code === 0) {
        this.dialog = true
        let headers = []
